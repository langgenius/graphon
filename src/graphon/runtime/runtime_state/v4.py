...
class Snapshot(FrameSnapshot):
    """Frame-scoped snapshot with workflow-wide root usage and step counters."""

    version: Literal["4.0"]


def dumps(state: RuntimeState) -> str:
    """Serialize one RuntimeState with the current version 4 schema.
    ...
    Returns:
        A validated version 4 JSON snapshot.
...
def loads(
    ...
    *,
    ...
    """Validate and restore an exact version 4 RuntimeState snapshot.

    Args:
        data: Serialized JSON whose version must be exactly ``"4.0"``.
